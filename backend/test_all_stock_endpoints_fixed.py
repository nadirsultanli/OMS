#!/usr/bin/env python3
"""
Comprehensive test script for all Stock Document API endpoints (FIXED VERSION)
Tests all CRUD operations, business logic, and edge cases with correct enum values
"""
...
class StockEndpointTestResults:
    ...
    def print_summary(self):
        print("\n" + "="*80)
        print("STOCK ENDPOINTS TEST RESULTS SUMMARY (FIXED VERSION)")
        print("="*80)
...
async def test_create_stock_doc(results: StockEndpointTestResults, client: httpx.AsyncClient):
    ...
    payload = {
        "doc_type": "TRF_WH",  # Transfer - using database enum value
...
async def test_get_stock_docs_by_type(results: StockEndpointTestResults, client: httpx.AsyncClient):
    ...
    endpoint = "/stock-docs/type/TRF_WH"  # Using database enum value
...
async def test_get_stock_docs_by_status(results: StockEndpointTestResults, client: httpx.AsyncClient):
    ...
    endpoint = "/stock-docs/status/open"  # Using lowercase enum value
...
async def test_cancel_stock_doc(results: StockEndpointTestResults, client: httpx.AsyncClient):
    ...
    # Create a fresh document for cancellation test
    create_payload = {
        "doc_type": "TRF_WH",
        "source_wh_id": SAMPLE_WAREHOUSE_ID_1,
        "dest_wh_id": SAMPLE_WAREHOUSE_ID_2,
        "notes": "Test document for cancellation",
        "stock_doc_lines": [
            {
                "variant_id": SAMPLE_VARIANT_ID_1,
                "quantity": "5.0",
    ...
    try:
        ...
        create_response = await client.post("/stock-docs/", json=create_payload)
        if create_response.status_code != 201:
            results.add_result(
                ...
                status_code=0,
                success=False,
                error_message=f"Failed to create document for cancellation test: {create_response.text}"
            )
            return
        
        cancel_doc_id = create_response.json().get("id")
        endpoint = f"/stock-docs/{cancel_doc_id}/cancel"
...
async def test_generate_doc_number(results: StockEndpointTestResults, client: httpx.AsyncClient):
    ...
    endpoint = "/stock-docs/generate-number/TRF_WH"  # Using database enum value
...
async def test_ship_transfer(results: StockEndpointTestResults, client: httpx.AsyncClient):
    ...
    # Create a fresh transfer document for shipping test
    create_payload = {
        "doc_type": "TRF_WH",
        "source_wh_id": SAMPLE_WAREHOUSE_ID_1,
        "dest_wh_id": SAMPLE_WAREHOUSE_ID_2,
        "notes": "Test transfer for shipping",
        "stock_doc_lines": [
            {
                "variant_id": SAMPLE_VARIANT_ID_1,
                "quantity": "3.0",
    ...
    try:
        ...
        if create_response.status_code != 201:
            results.add_result(
                ...
                status_code=0,
                success=False,
                error_message=f"Failed to create transfer for shipping test: {create_response.text}"
            )
            return
        
        ship_doc_id = create_response.json().get("id")
        endpoint = f"/stock-docs/{ship_doc_id}/ship"
...
async def test_receive_transfer(results: StockEndpointTestResults, client: httpx.AsyncClient):
    ...
    # Create a fresh transfer document for receiving test
    create_payload = {
        "doc_type": "TRF_WH",
        "source_wh_id": SAMPLE_WAREHOUSE_ID_1,
        "dest_wh_id": SAMPLE_WAREHOUSE_ID_2,
        "notes": "Test transfer for receiving",
        "stock_doc_lines": [
            {
                "variant_id": SAMPLE_VARIANT_ID_1,
                "quantity": "2.0",
    ...
    try:
        ...
        if create_response.status_code != 201:
            results.add_result(
                ...
                status_code=0,
                success=False,
                error_message=f"Failed to create transfer for receiving test: {create_response.text}"
            )
            return
        
        receive_doc_id = create_response.json().get("id")
        endpoint = f"/stock-docs/{receive_doc_id}/receive"
...
async def test_update_stock_doc(results: StockEndpointTestResults, client: httpx.AsyncClient):
    ...
    # Create a fresh document for update test
    create_payload = {
        "doc_type": "TRF_WH",
        "source_wh_id": SAMPLE_WAREHOUSE_ID_1,
        "dest_wh_id": SAMPLE_WAREHOUSE_ID_2,
        "notes": "Test document for update",
        "stock_doc_lines": [
            {
                "variant_id": SAMPLE_VARIANT_ID_1,
                "quantity": "8.0",
    ...
    try:
        ...
        if create_response.status_code != 201:
            results.add_result(
                ...
                status_code=0,
                success=False,
                error_message=f"Failed to create document for update test: {create_response.text}"
            )
            return
        
        update_doc_id = create_response.json().get("id")
        endpoint = f"/stock-docs/{update_doc_id}"
...
async def test_delete_stock_doc(results: StockEndpointTestResults, client: httpx.AsyncClient):
    ...
    # Create a fresh document for delete test
    create_payload = {
        "doc_type": "TRF_WH",
        "source_wh_id": SAMPLE_WAREHOUSE_ID_1,
        "dest_wh_id": SAMPLE_WAREHOUSE_ID_2,
    ...
    try:
        ...
        if create_response.status_code != 201:
            results.add_result(
                ...
                status_code=0,
                success=False,
                error_message=f"Failed to create document for delete test: {create_response.text}"
            )
            return
        
        delete_doc_id = create_response.json().get("id")
        endpoint = f"/stock-docs/{delete_doc_id}"
...
async def main():
    """Main test function"""
    print("🚀 Starting Stock Document API Endpoints Test Suite (FIXED VERSION)")
    ...
    filename = f"stock_endpoints_test_results_fixed_{timestamp}.json"
