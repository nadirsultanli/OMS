...
async def test_order_execute_endpoint():
    ...
    async with httpx.AsyncClient() as client:
        # Test 1: Try to execute a submitted order (should fail with 409 - not in transit)
        print(f"📡 Test 1: Executing submitted order (should fail - not in transit)")
        try:
            execute_data = {
            ...
            if response.status_code == 409:
                print("⚠️  EXPECTED: Order execution blocked - order not in transit")
                ...
                print("⚠️  EXPECTED: Order not found")
                results.add_result(
                    "/orders/execute", 
                    "POST", 
                    "FAIL", 
        ...
        # Test 2: Try to execute with invalid order ID (should fail with 404)
        ...
        # Test 3: Test with valid request structure but non-existent order
        print(f"\n📡 Test 3: Testing endpoint structure with valid request")
        try:
            execute_data = {
                "order_id": "3fa85f64-5717-4562-b3fc-2c963f66afa6",  # Example UUID
                "variants": [
                    {
            ...
            if response.status_code == 404:
                print("✅ EXPECTED: Order not found (valid request structure)")
                results.add_result(
                    "/orders/execute (structure test)", 
                    "POST", 
                    "PASS", 
                    response.status_code, 
                    response.text,
                    "Valid request structure, order not found"
                )
            else:
                print(f"📊 Response: {response.text}")
                results.add_result(
...
async def main():
    ...
    # Test the new order execution endpoint
    all_results.append(await test_order_execute_endpoint())
    ...
    print(f"✅ NEW: Order execution endpoint implemented and tested")
