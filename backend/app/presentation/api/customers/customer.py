...
async def delete_customer(customer_id: str, customer_service: CustomerService = Depends(get_customer_service), user: User = current_user):
    # Only Accounts can delete
    if user and user.role != "accounts":
        raise HTTPException(status_code=403, detail="Only Accounts can delete customers.")
    deleted_by = UUID(user.id) if user else None
...
async def approve_customer(customer_id: str, customer_service: CustomerService = Depends(get_customer_service), user: User = current_user):
    # Only Accounts can approve
    if user and user.role != "accounts":
        raise HTTPException(status_code=403, detail="Only Accounts can approve customers.")
    try:
        approved_by = UUID(user.id) if user else None
...
async def reject_customer(customer_id: str, customer_service: CustomerService = Depends(get_customer_service), user: User = current_user):
    # Only Accounts can reject
    if user and user.role != "accounts":
        raise HTTPException(status_code=403, detail="Only Accounts can reject customers.")
    try:
        rejected_by = UUID(user.id) if user else None
...
async def reassign_owner(customer_id: str, new_owner_sales_rep_id: UUID, customer_service: CustomerService = Depends(get_customer_service), user: User = current_user):
    # Only Accounts can reassign owner
    if user and user.role != "accounts":
        raise HTTPException(status_code=403, detail="Only Accounts can reassign owner.")
    reassigned_by = UUID(user.id) if user else None
...
async def inactivate_customer(customer_id: str, customer_service: CustomerService = Depends(get_customer_service), user: User = current_user):
    # Only Accounts can inactivate
    if user and user.role != "accounts":
        raise HTTPException(status_code=403, detail="Only Accounts can inactivate customers.")
    inactivated_by = UUID(user.id) if user else None
