...
async def get_current_user_subscription(
    ...
    try:
        ...
        # Direct approach: use the same method as debug endpoint
        ...
        logger.info(f"Direct query result: {len(result.data) if result.data else 0} records found")
        ...
        # Return the data directly without mapping
        sub_data = result.data[0]
        logger.info(f"Found subscription data: {sub_data}")
        ...
        subscription_dict = {
            ...
            'base_amount': float(sub_data['base_amount']),
            ...
            'ended_at': sub_data.get('ended_at'),
            'current_usage': sub_data.get('current_usage', {}),
            'created_at': sub_data['created_at'],
            'updated_at': sub_data['updated_at']
        }
        
        logger.info(f"Returning subscription data directly")
        return TenantSubscriptionResponse(**subscription_dict)
...
async def get_tenant_usage_summary(
    ...
    try:
        ...
        # First get the active subscription for the tenant
        ...
        if not sub_result.data:
            ...
            raise HTTPException(status_code=http_status.HTTP_404_NOT_FOUND, detail="No active subscription found for tenant")
        ...
        import traceback
        logger.error(
            "Failed to get tenant usage summary - unexpected error",
            user_id=str(current_user.id),
            tenant_id=tenant_id,
            error=str(e),
            traceback=traceback.format_exc()
